...
class ResClassifier1(nn.Module):
    def __init__(self, num_classes=12, num_layer=2, num_unit=2048, prob=0.5, middle=1000):
        ...
        self.layer1 = nn.Linear(num_unit, middle)
        self.batchnorm1 =  nn.BatchNorm1d(middle, affine=True)
        self.relu1 = nn.ReLU(inplace=True)

        self.dropout2 = nn.Dropout(p=prob)
        self.layer2 = nn.Linear(middle, middle)
        self.batchnorm2 =  nn.BatchNorm1d(middle, affine=True)
        self.relu2 = nn.ReLU(inplace=True)
        
        # self.dropout3 = nn.Dropout(p=prob)
        # self.layer3 = nn.Linear(num_unit, middle)
        # self.batchnorm3 =  nn.BatchNorm1d(middle, affine=True)
        # self.relu3 = nn.ReLU(inplace=True)

        self.layer3 = nn.Linear(middle, num_classes)
    ...
    def forward(self, x, reverse=False):
        ...
        z2 = self.dropout2(z1)
        z2 = self.layer2(z2)
        z2 = self.batchnorm2(z2)
        z2 = self.relu2(z2)
        
        # x = self.dropout3(x)
        # z3 = self.layer3(x)
        # x = self.batchnorm3(z2)
        # x = self.relu3(x)
